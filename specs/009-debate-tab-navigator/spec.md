...
- Q: What does “not voted yet” mean for a debate? → A: User has **not** cast **any** swipe vote on a binary card (**agree** or **disagree**) for that debate. **One vote per debate** (on the primary proposition card — prefer **agree**) moves it to **voted**; wildcard does not count.
...
- **FR-002b**: When **authenticated**, main debates screen calls the **user feed** API that returns **`new_debates`** and **`voted_debates`** (or equivalent split documented in contracts). **Completion** for bucketing: **at least one** swipe vote on any **agree**/**disagree** card (**one vote per debate**); **wildcard** does not count (matches mobile).
